...
{
	internal class AttributeRegex : Regex, RegexInterface
	{
		public AttributeRegex (LexerBase scanner) : base (@"(?<g1>\]\])|(?<g2>[a-zA-Z_]([a-zA-Z_]|[0-9]|[-\.])*|<[a-zA-Z_]([a-zA-Z_]|[0-9]|[-\. ])*>)|(?<g3>\=)|(?<g4>\""([^""]|\""\"")*\""|\'([^']|\'\')*\')|(?<g5>[\n\r])|(?<g6>.)", RegexOptions.ExplicitCapture)
		{
			Scanner = scanner;
		}

		public LexerBase Scanner { get; private set; }
		public static string Id { get; private set; } = "attribute_ctx";

		public delegate int scannerCallback (AttributeRegex regex, LexerBase scanner);
