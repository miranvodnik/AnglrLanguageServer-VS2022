...
{
	internal class ScannerIdRegex : Regex, RegexInterface
	{
		public ScannerIdRegex (LexerBase scanner) : base (@"(?<g1>\/\*)|(?<g2>\[\[)|(?<g3>\/\/[^\n]*)|(?<g4>[a-zA-Z_]([a-zA-Z_]|[0-9]|[-\.])*|<[a-zA-Z_]([a-zA-Z_]|[0-9]|[-\. ])*>)|(?<g5>[ \t]+)|(?<g6>[\n\r])", RegexOptions.ExplicitCapture)
		{
			Scanner = scanner;
		}

		public LexerBase Scanner { get; private set; }
		public static string Id { get; private set; } = "scanner_id_ctx";

		public delegate int scannerCallback (ScannerIdRegex regex, LexerBase scanner);
		...
		public (int, int) match (string currentLine, int currentPosition)
		{
			...
			case 3:
				...
				break;
			case 4:
				Scanner.pushScanner (ScannerPartRegex.Id, text);
