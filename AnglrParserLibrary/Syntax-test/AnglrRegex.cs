...
{
	internal class AnglrRegex : Regex, RegexInterface
	{
		public AnglrRegex (LexerBase scanner) : base (@"(?<g1>\/\*)|(?<g2>\[\[)|(?<g3>\/\/[^\n]*)|(?<g4>\|)|(?<g5>\,)|(?<g6>\:)|(?<g7>\;)|(?<g8>\{)|(?<g9>\})|(?<g10>\()|(?<g11>\))|(?<g12>\%\{)|(?<g13>\%\})|(?<g14>\[)|(?<g15>\])|(?<g16>\=)|(?<g17>\@\@)|(?<g18>\@)|(?<g19>\?)|(?<g20>\+)|(?<g21>\-)|(?<g22>\*)|(?<g23>\/)|(?<g24>\~\+)|(?<g25>\~\-)|(?<g26>\~\*)|(?<g27>\~\/)|(?<g28>%[ \t]*empty)|(?<g29>%[ \t]*terminal)|(?<g30>%[ \t]*general)|(?<g31>%[ \t]*declarations)|(?<g32>%[ \t]*regex)|(?<g33>%[ \t]*scanner)|(?<g34>%[ \t]*lexer)|(?<g35>%[ \t]*parser)|(?<g36>[a-zA-Z_]([a-zA-Z_]|[0-9]|[-\.])*|<[a-zA-Z_]([a-zA-Z_]|[0-9]|[-\. ])*>)|(?<g37>\""([^""]|\""\"")*\""|\'([^']|\'\')*\')|(?<g38>[0-9]+)|(?<g39>[ \t]+)|(?<g40>[\n\r])|(?<g41>.)", RegexOptions.ExplicitCapture)
		{
			Scanner = scanner;
		}
		...
		public delegate int scannerCallback (AnglrRegex regex, LexerBase scanner);
		...
		public (int, int) match (string currentLine, int currentPosition)
		{
			...
			case 31:
				...
				break;
			case 32:
				Scanner.pushScanner (RegexIdRegex.Id, text);
				result = AnglrDeclarations.tokens._regex_;
				break;
			case 33:
				Scanner.pushScanner (ScannerIdRegex.Id, text);
