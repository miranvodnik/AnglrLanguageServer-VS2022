...
{
	internal class RegexBlockRegex : Regex, RegexInterface
	{
		public RegexBlockRegex (LexerBase scanner) : base (@"(?<g1>\/\*)|(?<g2>\[\[)|(?<g3>\/\/[^\n]*)|(?<g4>[ \t\n]+)|(?<g5>\})|(?<g6>[a-zA-Z_]([a-zA-Z_]|[0-9]|[-\.])*|<[a-zA-Z_]([a-zA-Z_]|[0-9]|[-\. ])*>)", RegexOptions.ExplicitCapture)
		{
			Scanner = scanner;
		}

		public LexerBase Scanner { get; private set; }
		public static string Id { get; private set; } = "regex_block_ctx";

		public delegate int scannerCallback (RegexBlockRegex regex, LexerBase scanner);
		...
		public (int, int) match (string currentLine, int currentPosition)
		{
			...
			case 5:
				...
				break;
			case 6:
				Scanner.pushScanner (RegexBlockPartRegex.Id, text);
