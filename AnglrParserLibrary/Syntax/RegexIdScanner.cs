...
namespace Anglr.ScannerLib
{
	internal class RegexIdScanner : Regex, RegexInterface
	{
		...
		public static string Id { get; private set; } = "regex_id_scanner";

		public delegate int scannerCallback (RegexIdScanner regex, LexerBase scanner);
		...
		public (int, int) match (string currentLine, int currentPosition)
		{
			...
			case 3:
				...
				break;
			case 4:
				Scanner.pushScanner (RegexPartScanner.Id, text);
				result = AnglrDeclarations.tokens._identifier_;
				break;
			case 5:
				Scanner.pushScanner (RegexBlockScanner.Id, text);
